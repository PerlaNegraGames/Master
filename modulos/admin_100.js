...
export function iniciar(contenedor, db, miCarpeta) {
    ...
    contenedor.innerHTML = `
        <div style="background:#080808; border:1px solid var(--verde); padding:20px; border-radius:15px; text-align:center;">
            <h2 style="color:var(--verde); margin-top:0;">BINGO 100 NUMEROS</h2>
            <div style="display:flex; gap:10px; justify-content:center; margin-bottom:15px;">
                <button id="btn-sacar-100" class="btn-abrir" style="background:var(--cian); color:#000;">SACAR NÚMERO</button>
                <button id="btn-reset-100" class="btn-abrir" style="background:var(--rojo); color:#fff;">REINICIAR TABLERO</button>
            </div>
            <div id="ultimo-num-100" style="font-size:40px; font-weight:900; color:var(--amarillo); margin-bottom:15px;">--</div>
            <div id="tablero-grid-100" style="display:grid; grid-template-columns:repeat(10, 1fr); gap:5px; max-width:600px; margin:0 auto;"></div>
        </div>`;

    const refJuego = doc(db, "proyectos", miCarpeta, "juegosData", "bingo100");
    let numerosSalidos = [];
    ...
    onSnapshot(refJuego, (snap) => {
        if (snap.exists()) {
            ...
            const elemUltimo = document.getElementById('ultimo-num-100');
    ...
    function renderTablero(salidos) {
        ...
        const grid = document.getElementById('tablero-grid-100');
        if(grid) grid.innerHTML = html;
    }

    document.getElementById('btn-sacar-100').onclick = async () => {
    ...
    document.getElementById('btn-reset-100').onclick = async () => {
        if (confirm("¿Reiniciar juego de Bingo 100?")) {
